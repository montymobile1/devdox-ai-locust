...
class SafeFileCreator:
    ...
    def __init__(self, config: FileCreationConfig = None):
    ...
    async def create_temp_file(self, filename: str, content: str, temp_dir: Path) -> dict:
        ...
        permissions = 0o755 if file_extension in self.config.EXECUTABLE_EXTENSIONS else 0o644
    ...
    async def move_files_atomically(self, file_infos: List[dict], output_path: Path) -> List[dict]:
