...
class LocustTestEnhancer:
    ...
    async def _run_enhancement_pipeline(
        ...
        Returns:
            An ``EnhanceResult`` with the enhanced source and summary.
        """
        # -- Verbose: detailed analysis breakdown --
        if self._verbose:
            total_tasks = sum(
                len(c.task_methods) for c in analysis.user_classes
            )
            total_other = sum(
                len(c.other_methods) for c in analysis.user_classes
            )
            self._logger.info(
            ...
            has_seq = self._has_sequential_tasks(analysis)
            self._logger.info(
        ...
        if swagger_url:
            try:
                ...
                self._logger.info(
                    "Fetched API schema from %s", swagger_url
                )
                if self._verbose and api_schema_summary:
                    ep_count = api_schema_summary.count("\n- ")
                    self._logger.info(
                ...
                # Continue without schema -- it is optional context
        elif self._verbose:
            self._logger.info("[schema] no swagger URL provided — skipping")
        ...
        # -- Verbose: AI response section breakdown --
        section_keys = [
        ...
        if not any(new_sections.get(k, "").strip() for k in section_keys):
