...
class TestGetApiSchema:
    ...
    async def test_get_api_schema_from_url(self, mock_fetch_url):
        ...
        source = SwaggerProcessingRequest(swagger_url="https://api.example.com/swagger.json")
...
class TestFetchFromUrl:
    ...
    async def test_fetch_from_url_success(self, mock_httpx_client):
        ...
        with patch('devdox_ai_locust.utils.swagger_utils.httpx.AsyncClient') as mock_client_class:
    ...
    async def test_fetch_from_url_http_error(self):
        ...
        mock_error = httpx.HTTPStatusError("404", request=Mock(), response=mock_response)
...
class TestConcurrency:
    ...
    async def test_concurrent_url_fetches(self):
        ...
        with patch('devdox_ai_locust.utils.swagger_utils._fetch_from_url', side_effect=mock_fetch):
            tasks = [get_api_schema(SwaggerProcessingRequest(swagger_url=url)) for url in urls]
