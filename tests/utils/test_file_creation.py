...
class TestSafeFileCreatorIntegration:
    ...
    async def test_full_workflow(self, temp_dir):
        ...
        for filename, content in files_to_create.items():
            ...
            file_info = await creator.create_temp_file(
                clean_name, processed_content, staging_dir
            )
    ...
    async def test_concurrent_file_creation(self, temp_dir):
        ...
        async def create_file(i):
            ...
            if is_valid:
                return await creator.create_temp_file(
                    clean_name, processed_content, staging_dir
                )
            return None
    ...
    def test_custom_config_integration(self):
        ...
        # Test oversized file
        large_content = "x" * 200
        is_valid, _, processed_content = creator.validate_file(
            "large.py", large_content
        )
        assert is_valid
        assert len(processed_content) < len(large_content)
