...
class TestScenarioGeneratorSanitization:
    ...
    def generator(self):
        ...
        prompt_dir = (
            Path(__file__).parent.parent / "src" / "devdox_ai_locust" / "prompt"
        )
    ...
    def test_sanitize_identifier(self, generator, input_name, expected):
        ...
        assert (
            result == expected
        ), f"Input '{input_name}' should become '{expected}', got '{result}'"
    ...
    def test_operation_to_class_name_fallback(self, generator):
        ...
        assert (
            result == "GetUsers"
        ), f"Empty operation_id should fallback to method+path, got '{result}'"
    ...
    def test_generate_operation_id(self, generator, method, path, expected):
        ...
        assert (
            result == expected
        ), f"{method} {path} should become '{expected}', got '{result}'"

    @pytest.mark.parametrize(
        "operation_id,method,path,expected",
        [
            # With operation_id
    ...
    def test_get_endpoint_dir_name(
        self, generator, operation_id, method, path, expected
    ):
        ...
        assert (
            result == expected
        ), f"Operation '{operation_id}' or {method} {path} should become '{expected}', got '{result}'"
...
class TestIntegration:
    ...
    def test_same_input_produces_consistent_output(self):
        ...
        for name in test_names:
            ...
            assert (
                locust_file == scenario_file
            ), f"File names don't match for '{name}': {locust_file} vs {scenario_file}"
            ...
            assert (
                locust_class == scenario_class
            ), f"Class names don't match for '{name}': {locust_class} vs {scenario_class}"
    ...
    def test_generated_names_are_valid_python_identifiers(self):
        ...
        for input_name in test_inputs:
            ...
            assert (
                identifier.isidentifier() or identifier == "unnamed"
            ...
            assert (
                class_name.isidentifier() or class_name == "Unnamed"
...
"""
        ...
        assert (
            get_users.operation_id == "get_users"
        ), "Missing operationId should be generated"
        assert (
            post_users.operation_id == "createUser"
        ), "Existing operationId should be preserved"
        assert (
            get_user_id.operation_id == "get_users_id"
        ), "Path params should be included"
...
"""
        ...
        for endpoint in endpoints:
            assert (
                endpoint.operation_id is not None
            ), f"{endpoint.method} {endpoint.path} has None operation_id"
            assert (
                endpoint.operation_id != ""
            ), f"{endpoint.method} {endpoint.path} has empty operation_id"
            assert isinstance(
                endpoint.operation_id, str
            ), f"{endpoint.method} {endpoint.path} operation_id is not string"
...
class TestRealWorldExamples:
    ...
    @pytest.mark.parametrize(
        "tag_name,expected_file,expected_class",
        [
            # Common OpenAPI tags
            ("Users", "users", "Users"),
    ...
    def test_real_world_tags(self, generator, tag_name, expected_file, expected_class):
        ...
        ), f"File name for '{tag_name}': expected '{expected_file}', got '{file_name}'"
        assert (
            class_name == expected_class
        ), f"Class name for '{tag_name}': expected '{expected_class}', got '{class_name}'"

    @pytest.mark.parametrize(
        "operation_id,expected_file,expected_class",
        [
    ...
    def test_real_world_operations(
        self, generator, operation_id, expected_file, expected_class
    ):
        ...
        assert (
            file_name == expected_file
        ), f"File name for '{operation_id}': expected '{expected_file}', got '{file_name}'"
        assert (
            class_name == expected_class
        ), f"Class name for '{operation_id}': expected '{expected_class}', got '{class_name}'"
