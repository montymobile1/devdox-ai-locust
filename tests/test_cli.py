...
class TestCLI:
    ...
    def test_augment_command_help(self, cli_runner):
        """Test augment command help."""
        result = cli_runner.invoke(cli, ["augment", "--help"])
        assert result.exit_code == 0
        assert "Augment an existing Locust suite" in result.output
